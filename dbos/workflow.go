...
func checkGetResultExecution[R any](dbosCtx context.Context) (R, bool, error) {
	...
	if recordedOutputs != nil {
		...
		decodedOutput, err := decodeValue[R](dbosCtx.(*dbosContext).serializer, recordedOutputs.output, recordedOutputs.serialization)
		if err != nil {
...
func (h *workflowHandle[R]) processOutcome(outcome workflowOutcome[R], startTime, completedTime time.Time) (R, error) {
	...
	if isWithinWorkflow {
		...
		encodedOutput, encErr := encodeValue(h.dbosContext.(*dbosContext).serializer, decodedResult)
		if encErr != nil {
		...
		recordGetResultInput := recordOperationResultDBInput{
			...
			stepName:        "DBOS.getResult",
			serialization:   serializerName(h.dbosContext.(*dbosContext).serializer),
		}
...
func (h *workflowPollingHandle[R]) GetResult(opts ...GetResultOption) (R, error) {
	...
	if encodedStr != nil {
		var deserErr error
		typedResult, deserErr = decodeValue[R](h.dbosContext.(*dbosContext).serializer, encodedStr, serializerName(h.dbosContext.(*dbosContext).serializer))
		if deserErr != nil {
...
func RegisterWorkflow[P any, R any](ctx DBOSContext, fn Workflow[P, R], opts ...WorkflowRegistrationOption) {
	...
	typedErasedWorkflow := func(ctx DBOSContext, input any, inputSerialization string) (any, error) {
		...
		typedInput, err := decodeValue[P](getCustomSerializer(ctx), encodedInput, inputSerialization)
		if err != nil {
	...
	if registrationParams.cronSchedule != "" {
		...
		scheduledWfFunc := WorkflowFunc(func(ctx DBOSContext, input any) (any, error) {
			return typedErasedWorkflow(ctx, input, serializerName(getCustomSerializer(ctx)))
		})
		...
		go func() {
			...
			if outcome.needsDecoding {
				...
				if !ok { // Should never happen
					...
					var decodeErr error
					typedResult, decodeErr = decodeValue[R](getCustomSerializer(ctx), encodedResult, outcome.serialization)
					if decodeErr != nil {
...
func (c *dbosContext) RunWorkflow(_ DBOSContext, fn WorkflowFunc, input any, opts ...WorkflowOption) (WorkflowHandle[any], error) {
	...
	if params.alreadyEncodedInput {
		...
		var serErr error
		encodedInput, serErr = encodeValue(c.serializer, input)
		if serErr != nil {
	...
	workflowStatus := WorkflowStatus{
		...
		Serialization:      serializerName(c.serializer),
	}
	if isChildWorkflow {
	...
	go func() {
		...
		if errors.Is(err, &DBOSError{Code: ConflictingIDError}) {
			...
			encodedOutput, serErr := encodeValue(c.serializer, result)
			if serErr != nil {
...
func convertStepResult[R any](ctx DBOSContext, result any) (R, error) {
	...
	if _, ok := ctx.(*dbosContext); ok {
		...
		if checkpointed, ok := result.(stepCheckpointedOutcome); ok {
			...
			var decodeErr error
			typedResult, decodeErr = decodeValue[R](getCustomSerializer(ctx), encodedOutput, checkpointed.serialization)
			if decodeErr != nil {
				workflowID, _ := GetWorkflowID(ctx)
...
func (c *dbosContext) RunAsStep(_ DBOSContext, fn StepFunc, opts ...StepOption) (any, error) {
	...
	encodedStepOutput, serErr := encodeValue(c.serializer, stepOutput)
	if serErr != nil {
...
func (c *dbosContext) Send(_ DBOSContext, destinationID string, message any, topic string) error {
	...
	encodedMessage, err := encodeValue(c.serializer, message)
	if err != nil {
...
func Recv[R any](ctx DBOSContext, topic string, timeout time.Duration) (R, error) {
	...
	if _, ok := ctx.(*dbosContext); ok {
		...
		var decodeErr error
		typedMessage, decodeErr = decodeValue[R](getCustomSerializer(ctx), encodedMsg, serializerName(getCustomSerializer(ctx)))
		if decodeErr != nil {
...
func GetEvent[R any](ctx DBOSContext, targetWorkflowID, key string, timeout time.Duration) (R, error) {
	...
	if _, ok := ctx.(*dbosContext); ok {
		...
		var decodeErr error
		typedValue, decodeErr = decodeValue[R](getCustomSerializer(ctx), encodedValue, serializerName(getCustomSerializer(ctx)))
		if decodeErr != nil {
...
func WriteStream[P any](ctx DBOSContext, key string, value P) error {
	...
	encodedValue, err := encodeValue(getCustomSerializer(ctx), value)
	if err != nil {
...
func ReadStream[R any](ctx DBOSContext, workflowID string, key string) ([]R, bool, error) {
	...
	customSer := getCustomSerializer(ctx)
