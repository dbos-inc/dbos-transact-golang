...
func checkGetResultExecution[R any](dbosCtx context.Context) (R, bool, error) {
	...
	if recordedOutputs != nil {
		...
		decodedOutput, err := decodeValue[R](dbosCtx.(*dbosContext).serializer, recordedOutputs.output)
		if err != nil {
...
func (h *workflowPollingHandle[R]) GetResult(opts ...GetResultOption) (R, error) {
	...
	encodedStr, err := retryWithResult(ctx, func() (*string, error) {
	...
	if encodedStr != nil {
		var deserErr error
		typedResult, deserErr = decodeValue[R](h.dbosContext.(*dbosContext).serializer, encodedStr)
		if deserErr != nil {
...
func RegisterWorkflow[P any, R any](ctx DBOSContext, fn Workflow[P, R], opts ...WorkflowRegistrationOption) {
	...
	typedErasedWorkflow := WorkflowFunc(func(ctx DBOSContext, input any) (any, error) {
		...
		typedInput, err := decodeValue[P](getCustomSerializer(ctx), encodedInput)
		if err != nil {
		...
		go func() {
			...
			if outcome.needsDecoding {
				...
				if !ok { // Should never happen
					...
					var decodeErr error
					typedResult, decodeErr = decodeValue[R](getCustomSerializer(ctx), encodedResult)
					if decodeErr != nil {
...
func convertStepResult[R any](ctx DBOSContext, result any) (R, error) {
	...
	if _, ok := ctx.(*dbosContext); ok {
		...
		if checkpointed, ok := result.(stepCheckpointedOutcome); ok {
			...
			var decodeErr error
			typedResult, decodeErr = decodeValue[R](getCustomSerializer(ctx), encodedOutput)
			if decodeErr != nil {
				workflowID, _ := GetWorkflowID(ctx)
...
func Recv[R any](ctx DBOSContext, topic string, timeout time.Duration) (R, error) {
	...
	if _, ok := ctx.(*dbosContext); ok {
		...
		var decodeErr error
		typedMessage, decodeErr = decodeValue[R](getCustomSerializer(ctx), encodedMsg)
		if decodeErr != nil {
...
func GetEvent[R any](ctx DBOSContext, targetWorkflowID, key string, timeout time.Duration) (R, error) {
	...
	if _, ok := ctx.(*dbosContext); ok {
		...
		var decodeErr error
		typedValue, decodeErr = decodeValue[R](getCustomSerializer(ctx), encodedValue)
		if decodeErr != nil {
